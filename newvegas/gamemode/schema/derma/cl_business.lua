...
function PANEL:Init()
    ...
    -- Ensure menuWidth and menuHeight are set with default values if not configured
    local menuWidth = nut.config.menuWidth or 0.25
    local menuHeight = nut.config.menuHeight or 0.75
            ...
            for _, itemTable in SortedPairsByMemberValue(items, "name") do
                ...
                if (itemTable.faction) then
                    if (type(itemTable.faction) == "number" and itemTable.faction ~= LocalPlayer():Team()) then
                        allowed = false
                    elseif (type(itemTable.faction) == "table" and not table.HasValue(itemTable.faction, LocalPlayer():Team())) then
                        allowed = false
                    end
                end

                if (allowed and hook.Run("ShouldItemDisplay", itemTable) ~= false and not itemTable.noBusiness and (not itemTable.ShouldShowOnBusiness or (itemTable.ShouldShowOnBusiness and itemTable:ShouldShowOnBusiness(LocalPlayer()) ~= false))) then
                    ...
                    if (not self.categories[category2]) then
                        ...
                        if (itemTable.price and itemTable.price > 0) then
                            if (not nut.currency.IsSet()) then
                        ...
                        icon:SetToolTip("Name: " .. itemTable.name .. "\nDescription: "..itemTable:GetDesc().."\n"..cost.."\nThis item will be restocked in "..(string.NiceTime(itemTable.meta.lastrefresh + itemTable.meta.cooldown - os.time())))
