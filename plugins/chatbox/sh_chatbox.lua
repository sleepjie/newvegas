if (CLIENT) then
	...
	local PANEL = {}
		...
		function PANEL:Paint(p, w, h)
			--nut.util.drawBlur(p, 10)
			if (self.markup) then
				local alpha = 255
	...
	NUT_CVAR_CHATX = CreateClientConVar("forp_chatx", "64", true)
	NUT_CVAR_CHATY = CreateClientConVar("forp_chaty", "0.45", true)
	...
	cvars.AddChangeCallback("nut_chatx", function(conVar, oldValue, value)
		...
		if nut and nut.chat and nut.chat.panel and nut.chat.panel.frame then
	...
	function nut.chat.CreatePanels()
		if (!IsValid(nut.chat.panel.frame)) then
			...
			--frame:SetDrawBackground(true)

			frame.Paint = function(p, w, h)
				if nut and nut.chat and nut.chat.open then
